...
function Bebidas() {
    ...
    useEffect(() => {
        const fetchBebidas = async () => {
            try {
                ...
                if (!response.ok) throw new Error('Falha ao buscar as bebidas.');
                let data = await response.json();
                // Filtra para mostrar apenas produtos do tipo "Bebidas"
                data = data.filter(p => p.tipo === 'Bebidas');
                setBebidas(data);
            } catch (error) {
    ...
    if (loading) return <div style={{ padding: "40px", textAlign: 'center' }}><h1>Carregando bebidas...</h1></div>;
