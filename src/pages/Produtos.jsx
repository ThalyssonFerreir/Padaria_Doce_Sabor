...
function Produtos() {
    ...
    const { addToCart } = useCart();
        ...
        const fetchMercearia = async () => {
            try {
                ...
                if (!response.ok) throw new Error('Falha ao buscar os produtos de mercearia.');
                let data = await response.json();
                // Filtra para mostrar apenas produtos do tipo "Mercearia"
                data = data.filter(p => p.tipo === 'Mercearia');
                setMercearia(data);
            } catch (error) {
    ...
    if (loading) return <div style={{ padding: "40px", textAlign: 'center' }}><h1>Carregando produtos de mercearia...</h1></div>;
